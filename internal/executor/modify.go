...
func execUpdate(db database.Database, txnID int64, stmt *ast.UpdateStatement) (*Result, error) {
...
func execDelete(db database.Database, txnID int64, stmt *ast.DeleteStatement) (*Result, error) {
	ts, err := db.GetTableSchema(stmt.TableName)
	if err != nil {
		return nil, fmt.Errorf("table %q not found: %w", stmt.TableName, err)
	}
